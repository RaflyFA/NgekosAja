import { supabase } from "@/lib/supabase"
import { PropertyDetailPage } from "@/components/property-detail-page"

// PENTING: Paksa halaman ini untuk selalu ambil data terbaru (tidak cache)
export const revalidate = 0

export default async function Page({ params }: { params: Promise<{ id: string }> }) {

  // 1. Ambil ID dari URL
  const { id } = await params

  // 2. Ambil data kosan
  const { data: detailKos, error } = await supabase
    .from('boarding_houses')
    .select('*')
    .eq('id', id)
    .single()

  // 3. Cek Error / Kosong
  if (error || !detailKos) {
    console.error("❌ Gagal ambil data:", error)
    return (
      <div className="min-h-screen flex items-center justify-center">
        <h1 className="text-xl font-bold text-red-500">
          Kos tidak ditemukan atau ID salah.
        </h1>
      </div>
    )
  }

  // 4. Ambil owner data secara terpisah
  let ownerData = null
  if (detailKos.owner_id) {
    const { data: owner } = await supabase
      .from('profiles')
      .select('full_name, whatsapp, avatar_url')
      .eq('id', detailKos.owner_id)
      .single()
    ownerData = owner
  }

  // 5. Ambil reviews untuk kosan ini
  const { data: reviews } = await supabase
    .from('reviews')
    .select(`
      *,
      profiles(full_name)
    `)
    .eq('property_id', id)
    .order('created_at', { ascending: false })

  console.log("✅ Berhasil ambil detail kos:", detailKos.name)

  return (
    <main>
      {/* Kirim data kos, owner, dan reviews ke komponen tampilan */}
      <PropertyDetailPage
        dataKos={detailKos}
        ownerData={ownerData}
        reviewsData={reviews || []}
      />
    </main>
  )
}